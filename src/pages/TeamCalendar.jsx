...
import { getTeamById, getTeamMatches } from '../services/api';
...
function TeamCalendar() {
  ...
  useEffect(() => {
    const loadTeamName = async () => {
      try {
        ...
        setTeamName(data.name || data.team?.name);
      } catch (err) {
        console.error('Ошибка загрузки названия команды:', err);
      }
    };
    loadTeamName();
  }, [id]); 
  ...
  const loadMatches = useCallback(async (dateFrom = null, dateTo = null) => {
    try {
      setLoading(true);
      setError(null);
      const data = await getTeamMatches(id, dateFrom, dateTo);
